  ...
  Widget.Box({
    ...
    connections: [
      ...
      [
        Hyprland,
        (box) => {
          if (windowName && !App.getWindow(windowName).visible) return
      ...
      Separator({
        ...
        connections: [
          [
            Hyprland,
            (box) => {
              box.visible = box.get_parent().children.at(-1).children.length > 0
            },
          ],
        ],
      }),
      Taskbar({
        ...
        item: ({ iconName }, { address, title }) =>
          AppButton({
            ...
            tooltipText: title,
            className:
              Hyprland.active.client.address === address.substring(2)
                ? "focused"
                : "nonfocused",
            onClicked: () =>
              execAsync(
